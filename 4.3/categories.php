...
$create_table_sql = "create table categories (CategoryID int(11),  Categoryname varchar(50), Description varchar(100), Picture int(11))";
...
if (strlen($act) == 0 || $act == "del" || $act == "edit") {
  ...
  while ($o = mysql_fetch_object($r)) {
    if (isset($_REQUEST{'CategoryID'}) && $_REQUEST{'CategoryID'}  == $o->Proid) $chg = " style='background-color:#f9f9f9"; else $chg = " readonly style='background-color:#ffffdd";
    echo "<tr><form action='' method=post>
      ...
      <td><input name=Picture size=20 value='". $o->Picture . "' $chg;text-align:right'></td>
      <td>";
  ...
  echo "<tr><form action='' method=post><td><input name=CategoryID size=5></td><td><input name=Categoryname size=40></td><td><input name=Description size=20></td><td><input name=Picture size=20></td><td><input type=submit name=action value='add' style='height:26'></td></tr>
  </form></table>";
...
# ÊèÇ¹à¾ÔèÁ¢éÍÁÙÅ
if ($act == "add") {
  $q  = "insert into categories values('". $_REQUEST{'CategoryID'} . "','". $_REQUEST{'Categoryname'} . "',". $_REQUEST{'Description'} . ",". $_REQUEST{'Picture'} . ")";
  $conn = mysql_connect("$host","$user","$password");
...
# ÊèÇ¹á¡éä¢¢éÍÁÙÅ
if ($act == "edit : confirm") {
  $q  = "update $tb set Categoryname ='". $_REQUEST{'Categoryname'} . "', Description = ". $_REQUEST{'Description'} . " , Picture = ". $_REQUEST{'Picture'} . " where CategoryID ='" . $_REQUEST{'CategoryID'} . "'";
// die($q);
  $conn = mysql_connect("$host","$user","$password");
  $r = mysql_db_query($db,$q);   
