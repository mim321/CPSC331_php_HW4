<html><head>
<script src="dmenu_array.php"></script>
<script src="dmenu.php"></script>
</head>
<?php 
session_start();
...
$create_table_sql = "create table test (Proid varchar(20),  Proname varchar(20), Proamount int(20), Proprice double)";
...
# ส่วนเพิ่มข้อมูล
if ($act == "add") {
  $q  = "insert into test values('". $_REQUEST{'Proid'} . "','". $_REQUEST{'Proname'} . "',". $_REQUEST{'Proamount'} . ",". $_REQUEST{'Proprice'} . ")";
  $conn = mysql_connect("$host","$user","$password");
...
# ส่วนลบข้อมูล
if ($act == "del : confirm") {
  $q  = "delete from test where Proid ='". $_REQUEST{'Proid'} . "'";
  $conn = mysql_connect("$host","$user","$password");
  $r = mysql_db_query($db,$q);   
